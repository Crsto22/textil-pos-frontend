...
interface ExportPagoReporteExcelParams extends PagoFilters {
  idVenta?: number | null
  desde?: string
  hasta?: string
}

function buildQueryString(params: ExportPagoReporteExcelParams): string {
...
async function getErrorMessage(response: Response): Promise<string> {
  ...
  return "No se pudo exportar el reporte de pagos en Excel"
}

function buildFallbackFilename() {
  const now = new Date()
  const datePart = now.toISOString().slice(0, 19).replaceAll("-", "").replaceAll(":", "").replace("T", "_")
  return `reporte_pagos_${datePart}.xlsx`
}

export function usePagoReporteExcel() {
  const [isExportingExcel, setIsExportingExcel] = useState(false)

  const exportReporteExcel = useCallback(
    async (params: ExportPagoReporteExcelParams): Promise<boolean> => {
      ...
      try {
        ...
        const endpoint = queryString
          ? `/api/pago/reporte/excel?${queryString}`
          : "/api/pago/reporte/excel"
        ...
        toast.error("Error de red al exportar el reporte Excel")
        return false
      } finally {
