...
function getSpecialSchedules(turno: Turno) {
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              turnos.map((turno) => {
                const specialSchedules = getSpecialSchedules(turno)
                    ...
                    <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">
                      <div className="space-y-1">
                        ...
                        {specialSchedules.length > 0 ? (
                          <div className="space-y-0.5 whitespace-normal text-[11px]">
                            {specialSchedules.map((horario) => (
                              <div
                                key={horario.dia}
                                className="text-amber-600 dark:text-amber-300"
                              >
                                {DIA_LABEL[horario.dia as DiaSemana] ?? horario.dia}:{" "}
                      ...
                      <div className="flex flex-wrap gap-1">
                        {Array.isArray(turno.dias) && turno.dias.length > 0 ? (
                          turno.dias.map((dia) => (
                            <span
                              key={dia}
                              className="inline-flex items-center rounded bg-blue-50 px-1.5 py-0.5 text-[10px] font-semibold text-blue-700 dark:bg-blue-500/10 dark:text-blue-300"
                            >
