...
interface EmpresaUbigeoFieldsProps {
    enabled: boolean
    form: EmpresaLocationFields
    idPrefix: string
    onChange: (next: EmpresaLocationFields) => void
}

export function EmpresaUbigeoFields({
    enabled,
    form,
    idPrefix,
    onChange,
}: EmpresaUbigeoFieldsProps) {
    ...
    useEffect(() => {
        ...
        setSelection((previous) =>
            arePeruUbigeoSelectionsEqual(previous, nextSelection)
                ? previous
                : nextSelection
        )
    ...
    const selectedDepartment = useMemo(
        () =>
            catalog.find(
                (department) => department.code === selection.departmentCode
            ) ?? null,
    ...
    const selectedProvince = useMemo(
        () =>
            provinceOptions.find(
                (province) => province.code === selection.provinceCode
            ) ?? null,
    ...
    const handleProvinceChange = (provinceCode: string) => {
        ...
        const province =
            selectedDepartment.provinces.find(
        ...
        <>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div className="grid gap-2">
                    <Label htmlFor={`${idPrefix}-departamento-empresa`}>
                        Departamento
                    </Label>
                    <Select
                    ...
                    >
                        <SelectTrigger
                            className="w-full"
                            id={`${idPrefix}-departamento-empresa`}
                        >
                            <SelectValue
                                placeholder={
                                    isLoadingCatalog
                ...
                <div className="grid gap-2">
                    <Label htmlFor={`${idPrefix}-provincia-empresa`}>
                        Provincia
                    </Label>
                    <Select
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div className="grid gap-2">
                    <Label htmlFor={`${idPrefix}-distrito-empresa`}>
                        Distrito
                    </Label>
                    <Select
                ...
                <div className="grid gap-2">
                    <Label htmlFor={`${idPrefix}-ubigeo-empresa`}>Ubigeo</Label>
                    <Input
                        id={`${idPrefix}-ubigeo-empresa`}
                        value={form.ubigeo}
