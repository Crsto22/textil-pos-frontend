...
const ALLOWED_QUERY_KEYS = ["codigoBarras", "idSucursal"] as const
...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      backendRes = await fetch(`${BACKEND_URL}/api/variante/escanear${queryString}`, {
        headers,
        cache: "no-store",
      })
    } catch {
    ...
    if (!backendRes.ok) {
      const message = getErrorMessage(payload, "Error al escanear variante")
      return NextResponse.json({ message }, { status: backendRes.status })
    }

    return NextResponse.json(payload, { status: backendRes.status })
  } catch (error) {
    console.error("[VARIANTE/ESCANEAR]", error)
