...
export async function PATCH(request: NextRequest) {
  try {
    ...
    try {
      backendRes = await fetch(`${BACKEND_URL}/api/variante/ofertas/lote`, {
        method: "PATCH",
        headers,
        body,
      })
    ...
    if (!backendRes.ok) {
      const message = getErrorMessage(payload, "Error al actualizar ofertas por lote")
      return NextResponse.json({ message }, { status: backendRes.status })
    }

    return NextResponse.json(payload, { status: backendRes.status })
  } catch (error) {
    console.error("[VARIANTE/OFERTAS/LOTE]", error)
