...
export async function GET(
  ...
  try {
    ...
    try {
      backendRes = await fetch(`${BACKEND_URL}/api/cotizacion/detalle/${encodeURIComponent(id)}`, {
        headers,
        cache: "no-store",
      })
    } catch {
    ...
    if (!backendRes.ok) {
      const message =
        typeof payload.message === "string"
          ? payload.message
          : "No se pudo obtener el detalle de la cotizacion"
    ...
    console.error("[COTIZACION/DETALLE]", error)
