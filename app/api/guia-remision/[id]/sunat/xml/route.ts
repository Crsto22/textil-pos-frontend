...
async function parseErrorResponse(response: Response) {
  ...
  return { message: text || "No se pudo descargar el XML de SUNAT" }
}
  ...
  try {
    ...
    try {
      backendRes = await fetch(
        `${BACKEND_URL}/api/guia-remision/${encodeURIComponent(id)}/sunat/xml`,
        {
          method: "GET",
          headers,
          cache: "no-store",
    ...
    const contentDisposition =
      ...
      `attachment; filename="guia_remision_${encodeURIComponent(id)}.xml"`
    ...
    console.error("[GUIA-REMISION/SUNAT/XML]", error)
