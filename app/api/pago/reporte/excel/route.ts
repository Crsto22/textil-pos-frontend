...
async function parseErrorResponse(response: Response) {
  ...
  return { message: text || "No se pudo descargar el reporte de pagos en Excel" }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      backendRes = await fetch(`${BACKEND_URL}/api/pago/reporte/excel${queryString}`, {
        method: "GET",
        headers,
        cache: "no-store",
      })
    ...
    const contentDisposition =
      ...
      `attachment; filename="reporte_pagos_${Date.now()}.xlsx"`
    ...
    console.error("[PAGO/REPORTE/EXCEL]", error)
