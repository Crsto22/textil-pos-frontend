...
export async function POST(
  ...
  try {
    ...
    try {
      backendRes = await fetch(`${BACKEND_URL}/api/venta/${encodeURIComponent(id)}/anular`, {
        method: "POST",
        headers,
        body,
      })
    ...
    if (!backendRes.ok) {
      return NextResponse.json(
        { message: getErrorMessage(payload, "No se pudo anular la venta") },
    ...
    console.error("[VENTA/ANULAR]", error)
