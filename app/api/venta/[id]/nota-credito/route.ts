  ...
  try {
    ...
    try {
      backendRes = await fetch(
        `${BACKEND_URL}/api/venta/${encodeURIComponent(id)}/nota-credito`,
        {
          method: "POST",
          headers,
          body,
    ...
    if (!backendRes.ok) {
      return NextResponse.json(
        { message: getErrorMessage(payload, "No se pudo registrar la nota de credito") },
    ...
    console.error("[VENTA/NOTA_CREDITO]", error)
