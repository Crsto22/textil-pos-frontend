...
async function parseErrorResponse(response: Response) {
  ...
  return { message: text || "No se pudo obtener el ticket en PDF" }
}
  ...
  try {
    ...
    try {
      backendRes = await fetch(
        `${BACKEND_URL}/api/venta/${encodeURIComponent(id)}/comprobante/ticket`,
        {
          method: "GET",
          headers,
          cache: "no-store",
    ...
    const contentDisposition =
      ...
      `inline; filename=\"ticket_venta_${encodeURIComponent(id)}.pdf\"`
    ...
    console.error("[VENTA/COMPROBANTE/TICKET]", error)
