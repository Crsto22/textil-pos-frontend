...
async function parseErrorResponse(response: Response) {
  ...
  return { message: text || "No se pudo descargar el comprobante en PDF" }
}
  ...
  try {
    ...
    try {
      backendRes = await fetch(
        `${BACKEND_URL}/api/venta/${encodeURIComponent(id)}/comprobante/pdf`,
        {
          method: "GET",
          headers,
          cache: "no-store",
    ...
    console.error("[VENTA/COMPROBANTE/PDF]", error)
