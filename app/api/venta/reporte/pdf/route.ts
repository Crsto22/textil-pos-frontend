...
async function parseErrorResponse(response: Response) {
  ...
  return { message: text || "No se pudo descargar el reporte de ventas en PDF" }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      backendRes = await fetch(`${BACKEND_URL}/api/venta/reporte/pdf${queryString}`, {
        method: "GET",
        headers,
        cache: "no-store",
      })
    ...
    console.error("[VENTA/REPORTE/PDF]", error)
