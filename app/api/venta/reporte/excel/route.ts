...
async function parseErrorResponse(response: Response) {
  ...
  return { message: text || "No se pudo exportar el reporte en Excel" }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      backendRes = await fetch(`${BACKEND_URL}/api/venta/reporte/excel${queryString}`, {
        method: "GET",
        headers,
        cache: "no-store",
      })
    ...
    const contentDisposition =
      ...
      `attachment; filename="reporte_ventas_${Date.now()}.xlsx"`
    ...
    console.error("[VENTA/REPORTE/EXCEL]", error)
