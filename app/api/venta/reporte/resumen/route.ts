...
const ALLOWED_QUERY_KEYS = ["filtro", "idSucursal"] as const
...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      backendRes = await fetch(`${BACKEND_URL}/api/venta/reporte/resumen${queryString}`, {
        method: "GET",
        headers,
        cache: "no-store",
      })
    ...
    console.error("[VENTA/REPORTE/RESUMEN]", error)
