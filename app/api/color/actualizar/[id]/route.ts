  ...
  try {
    ...
    try {
      backendRes = await fetch(
        `${BACKEND_URL}/api/color/actualizar/${encodeURIComponent(id)}`,
        {
          method: "PUT",
          headers,
          body,
    ...
    if (!backendRes.ok) {
      ...
      let message = "Error al actualizar color"
      try {
    ...
    try {
      ...
      data = { message: text || "Color actualizado exitosamente" }
    }

    return NextResponse.json(data, { status: 200 })
  } catch (error) {
    console.error("[COLOR/ACTUALIZAR]", error)
    return NextResponse.json(
