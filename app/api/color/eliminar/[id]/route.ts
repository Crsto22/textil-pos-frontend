  ...
  try {
    ...
    try {
      backendRes = await fetch(
        `${BACKEND_URL}/api/color/eliminar/${encodeURIComponent(id)}`,
        {
          method: "DELETE",
          headers,
        }
    ...
    if (!backendRes.ok) {
      ...
      let message = "Error al eliminar color"
      try {
    ...
    const text = await backendRes.text()
    let message = "Color eliminado correctamente"
    try {
      const json = JSON.parse(text)
    ...
    console.error("[COLOR/ELIMINAR]", error)
    return NextResponse.json(
