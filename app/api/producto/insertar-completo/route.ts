...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      backendRes = await fetch(`${BACKEND_URL}/api/producto/insertar-completo`, {
        method: "POST",
        headers,
        body,
      })
    ...
    if (!backendRes.ok) {
      let message = "Error al crear producto completo"
      try {
    ...
    try {
      ...
      data = { message: text || "Producto creado exitosamente" }
    }

    return NextResponse.json(data, { status: backendRes.status || 201 })
  } catch (error) {
    console.error("[PRODUCTO/INSERTAR-COMPLETO]", error)
    return NextResponse.json(
