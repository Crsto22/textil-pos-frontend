...
const ALLOWED_QUERY_KEYS = ["idSucursal", "q"] as const
...
export async function GET(request: NextRequest) {
  try {
    ...
    const incomingSearchParams = new URL(request.url).searchParams
    const idSucursal = incomingSearchParams.get("idSucursal")?.trim()
    if (!idSucursal) {
      return NextResponse.json({ message: "idSucursal es obligatorio" }, { status: 400 })
    }
    ...
    try {
      backendRes = await fetch(`${BACKEND_URL}/api/sucursal-stock/listar${queryString}`, {
        headers,
        cache: "no-store",
      })
    } catch {
    ...
    if (!backendRes.ok) {
      const message =
        payload && typeof payload === "object" && "message" in payload && typeof (payload as Record<string, unknown>).message === "string"
          ...
          : "Error al listar stock por sucursal"
      return NextResponse.json({ message }, { status: backendRes.status })
    }

    return NextResponse.json(payload, { status: 200 })
  } catch (error) {
    console.error("[SUCURSAL-STOCK/LISTAR]", error)
