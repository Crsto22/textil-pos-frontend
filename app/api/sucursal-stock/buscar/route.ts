...
const ALLOWED_QUERY_KEYS = ["idSucursal", "idProductoVariante"] as const
...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      backendRes = await fetch(`${BACKEND_URL}/api/sucursal-stock/buscar${queryString}`, {
        headers,
        cache: "no-store",
      })
    } catch {
    ...
    if (!backendRes.ok) {
      const message =
        payload && typeof payload === "object" && "message" in payload && typeof (payload as Record<string, unknown>).message === "string"
          ...
          : "Error al buscar stock por sucursal"
      return NextResponse.json({ message }, { status: backendRes.status })
    }

    return NextResponse.json(payload, { status: 200 })
  } catch (error) {
    console.error("[SUCURSAL-STOCK/BUSCAR]", error)
