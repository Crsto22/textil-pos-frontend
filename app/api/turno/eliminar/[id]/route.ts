  ...
  try {
    ...
    try {
      backendRes = await fetch(
        `${BACKEND_URL}/api/turno/eliminar/${encodeURIComponent(id)}`,
    ...
    if (!backendRes.ok) {
      ...
      let message = "Error al eliminar turno"
      try {
    ...
    const text = await backendRes.text()
    let message = "Turno eliminado logicamente"
    try {
      const json = JSON.parse(text)
    ...
    console.error("[TURNO/ELIMINAR]", error)
    return NextResponse.json(
