...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      backendRes = await fetch(`${BACKEND_URL}/api/turno/crear`, {
        method: "POST",
        headers,
        body,
      })
    ...
    if (!backendRes.ok) {
      ...
      let message = "Error al crear turno"
      try {
    ...
    try {
      ...
      data = { message: text || "Turno creado exitosamente" }
    }

    return NextResponse.json(data, { status: backendRes.status || 201 })
  } catch (error) {
    console.error("[TURNO/CREAR]", error)
    return NextResponse.json(
