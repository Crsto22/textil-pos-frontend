...
async function parseErrorResponse(response: Response) {
  ...
  return { message: text || "No se pudo descargar el PDF de la nota de credito" }
}
  ...
  try {
    ...
    try {
      backendRes = await fetch(
        `${BACKEND_URL}/api/nota-credito/${encodeURIComponent(id)}/comprobante/pdf`,
        {
          method: "GET",
          headers,
          cache: "no-store",
    ...
    const contentDisposition =
      ...
      `attachment; filename="nota_credito_${encodeURIComponent(id)}.pdf"`
    ...
    console.error("[NOTA_CREDITO/COMPROBANTE/PDF]", error)
