...
async function parseErrorResponse(response: Response) {
  ...
  return { message: text || "No se pudo descargar el CDR de baja de SUNAT" }
}
...
export async function GET(
  ...
  try {
    ...
    const backendUrl = new URL(
      `${BACKEND_URL}/api/nota-credito/${encodeURIComponent(id)}/sunat/baja/cdr`
    )
    if (formato) backendUrl.searchParams.set("formato", formato)
    ...
    const fallbackExt = formato === "xml" ? "xml" : "zip"
    const contentDisposition =
      backendRes.headers.get("content-disposition") ??
      `attachment; filename="cdr_baja_nota_credito_${encodeURIComponent(id)}.${fallbackExt}"`
    ...
    console.error("[NOTA_CREDITO/SUNAT/BAJA/CDR]", error)
