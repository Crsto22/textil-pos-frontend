...
export async function GET(
  ...
  try {
    ...
    const backendUrl = new URL(
      `${BACKEND_URL}/api/nota-credito/${encodeURIComponent(id)}/sunat/cdr`
    )
    if (formato) {
    ...
    const contentDisposition =
      ...
      `attachment; filename="cdr_nota_credito_${encodeURIComponent(id)}.${fallbackExt}"`
    ...
    console.error("[NOTA_CREDITO/SUNAT/CDR]", error)
