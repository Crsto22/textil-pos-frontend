  ...
  try {
    ...
    try {
      backendRes = await fetch(
        `${BACKEND_URL}/api/talla/eliminar/${encodeURIComponent(id)}`,
        {
          method: "DELETE",
          headers,
        }
    ...
    if (!backendRes.ok) {
      ...
      let message = "Error al eliminar talla"
      try {
    ...
    const text = await backendRes.text()
    let message = "Talla eliminada correctamente"
    try {
      const json = JSON.parse(text)
    ...
    console.error("[TALLA/ELIMINAR]", error)
    return NextResponse.json(
