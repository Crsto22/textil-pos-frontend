...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      backendRes = await fetch(`${BACKEND_URL}/api/talla/insertar`, {
        method: "POST",
        headers,
        body,
      })
    ...
    if (!backendRes.ok) {
      ...
      let message = "Error al crear talla"
      try {
    ...
    try {
      ...
      data = { message: text || "Talla creada exitosamente" }
    }

    return NextResponse.json(data, { status: backendRes.status || 201 })
  } catch (error) {
    console.error("[TALLA/INSERTAR]", error)
    return NextResponse.json(
