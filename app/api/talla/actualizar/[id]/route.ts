  ...
  try {
    ...
    try {
      backendRes = await fetch(
        `${BACKEND_URL}/api/talla/actualizar/${encodeURIComponent(id)}`,
        {
          method: "PUT",
          headers,
          body,
    ...
    if (!backendRes.ok) {
      ...
      let message = "Error al actualizar talla"
      try {
    ...
    try {
      ...
      data = { message: text || "Talla actualizada exitosamente" }
    }

    return NextResponse.json(data, { status: 200 })
  } catch (error) {
    console.error("[TALLA/ACTUALIZAR]", error)
    return NextResponse.json(
