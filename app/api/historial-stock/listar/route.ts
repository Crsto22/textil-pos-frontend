...
const ALLOWED_QUERY_KEYS = ["idSucursal", "tipoMovimiento", "page", "size"] as const
...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      backendRes = await fetch(`${BACKEND_URL}/api/historial-stock/listar${queryString}`, {
        headers,
        cache: "no-store",
      })
    } catch {
    ...
    if (!backendRes.ok) {
      const message =
        payload && typeof payload === "object" && "message" in payload && typeof (payload as Record<string, unknown>).message === "string"
          ...
          : "Error al listar historial de stock"
      return NextResponse.json({ message }, { status: backendRes.status })
    }

    return NextResponse.json(payload, { status: 200 })
  } catch (error) {
    console.error("[HISTORIAL-STOCK/LISTAR]", error)
