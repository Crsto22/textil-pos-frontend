  ...
  try {
    ...
    try {
      backendRes = await fetch(
        `${BACKEND_URL}/api/historial-stock/variante/${encodeURIComponent(id)}${queryString}`,
    ...
    if (!backendRes.ok) {
      const message =
        payload && typeof payload === "object" && "message" in payload && typeof (payload as Record<string, unknown>).message === "string"
          ...
          : "Error al obtener historial de stock de la variante"
      return NextResponse.json({ message }, { status: backendRes.status })
    }

    return NextResponse.json(payload, { status: 200 })
  } catch (error) {
    console.error("[HISTORIAL-STOCK/VARIANTE]", error)
