  ...
  try {
    ...
    try {
      backendRes = await fetch(
        `${BACKEND_URL}/api/categoria/actualizar/${encodeURIComponent(id)}`,
        {
          method: "PUT",
          headers,
          body,
    ...
    if (!backendRes.ok) {
      ...
      let message = "Error al actualizar categoria"
      try {
    ...
    try {
      ...
      data = { message: text || "Categoria actualizada exitosamente" }
    }

    return NextResponse.json(data, { status: 200 })
  } catch (error) {
    console.error("[CATEGORIA/ACTUALIZAR]", error)
    return NextResponse.json(
