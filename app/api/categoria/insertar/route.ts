...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      backendRes = await fetch(`${BACKEND_URL}/api/categoria/insertar`, {
        method: "POST",
        headers,
        body,
      })
    ...
    if (!backendRes.ok) {
      ...
      let message = "Error al crear categoria"
      try {
    ...
    try {
      ...
      data = { message: text || "Categoria creada exitosamente" }
    }

    return NextResponse.json(data, { status: backendRes.status || 201 })
  } catch (error) {
    console.error("[CATEGORIA/INSERTAR]", error)
    return NextResponse.json(
