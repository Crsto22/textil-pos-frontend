...
const ALLOWED_QUERY_KEYS = ["idSucursal", "plataforma", "activo"] as const
...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      backendRes = await fetch(`${BACKEND_URL}/api/canal-venta/listar${queryString}`, {
        headers,
        cache: "no-store",
      })
    } catch {
    ...
    if (!backendRes.ok) {
      const message =
        payload && typeof payload === "object" && "message" in payload && typeof (payload as Record<string, unknown>).message === "string"
          ...
          : "Error al listar canales de venta"
      return NextResponse.json({ message }, { status: backendRes.status })
    }

    return NextResponse.json(payload, { status: 200 })
  } catch (error) {
    console.error("[CANAL-VENTA/LISTAR]", error)
